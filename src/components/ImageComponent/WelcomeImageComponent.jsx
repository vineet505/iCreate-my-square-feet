...
import { AddWelcomeCardImageUrl } from "@/static/ApiEndpoints";
import { getBase64 } from "@/utility/utils";

export const WelcomeImageComponent = ({ ImageUrl, card_id, imageId }) => {
  ...
  const props = {
    name: "file",
    action:  `${AddWelcomeCardImageUrl}`,
  };

  props.customRequest = ({ file, onSuccess, onError }) => {
    if (isCancel) return;
    const formData = new FormData();
    formData.append("welcome_card_image", file); 
    formData.append("card_id", card_id);

    fetch(`${AddWelcomeCardImageUrl}`, {
      method: "POST",
      body: formData,
      headers: {
