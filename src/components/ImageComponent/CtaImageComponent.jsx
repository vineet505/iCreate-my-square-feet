...
import { AddCTACardImageUrl } from "@/static/ApiEndpoints";
import { getBase64 } from "@/utility/utils";

export const CtaImageComponent = ({ ImageUrl, card_id, imageId }) => {
  ...
  const props = {
    name: "file",
    action:  `${AddCTACardImageUrl}`,
  };

  props.customRequest = ({ file, onSuccess, onError }) => {
    if (isCancel) return;
    const formData = new FormData();
    formData.append("cta_image", file); 
    formData.append("card_id", card_id);

    fetch(`${AddCTACardImageUrl}`, {
      method: "POST",
      body: formData,
      headers: {
