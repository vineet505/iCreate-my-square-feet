...
import { AddAdsImageUrl } from "@/static/ApiEndpoints";
import { getBase64 } from "@/utility/utils";

export const AdsImageComponent = ({ ImageUrl, card_id, imageId }) => {
  ...
  const props = {
    name: "file",
    action:  `${AddAdsImageUrl}`,
  };

  props.customRequest = ({ file, onSuccess, onError }) => {
    if (isCancel) return;
    const formData = new FormData();
    formData.append("ads_image", file); 
    formData.append("card_id", card_id);

    fetch(`${AddAdsImageUrl}`, {
      method: "POST",
      body: formData,
      headers: {
